...
- **Skill Bank — backbone expansion (2026-05-23, public-safe summary):** Local _KnowledgeWiki Skill Bank grew with agent-neutral procedures including **workflow audit**, **safe memory update**, **project manifest** (per-project explainer), **skill discovery** (live registry via `skills/README.md`), and **skill opportunity review** (controlled growth, no skill sprawl). Agents should treat **`skills/README.md` as the live skill registry**; per-agent quick tables are shortcuts only. **Operational rule:** stabilize existing skills before adding new ones. Detailed validation and project manifests remain **local only**.
